...
import com.google.android.gms.samples.wallet.service.eWAYPaymentService;
...
public class CheckoutActivity extends AppCompatActivity{
  ...
  public eWAYPaymentService eWAYPaymentService;
   ...
   */
  private void handlePaymentSuccess(@NotNull PaymentData paymentData) {
    ...
    try {
      ...
      //Make request to Rapid, pass the token and total price
      String response = eWAYPaymentService.MakePayment(token, totalPrice);

      if (!response.isEmpty()){
        Log.d("Google Pay token: ", token);
        RedirectToShowResult(response); //Redirect to show the result
      }
  ...
  public void RedirectToShowResult(String response){
  ...
  public void requestPayment(View view) {
    ...
    try {
      ...
      // onActivityResult will be ca1lled with the result.
      if (request != null) {
